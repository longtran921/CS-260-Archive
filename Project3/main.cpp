...
// advanced tests
    ...
    std::cout << "Done testing mixed inserts and deletes\n" << std::endl;
#endif // TEST_MIXED
...
// thinking test
    ...
    for (int i = 0; i < CAT; i++) {
        catText.addTail(catLetters[i]);
    }

    for (int i = 0; i < CAT; i++) {
        dogText.addTail(dogLetters[i]);
    }

    catText.append(dogText);

    catText.findNext('T'); catText.findNext('T');
    catText.insertLast(' '); catText.insertLast('a');
    catText.insertLast('n'); catText.insertLast('d'); catText.insertLast(' '); catText.insertLast('t');
    catText.findNext('T'); catText.findNext('T'); catText.removeLast();
    catText.insertLast('a'); catText.insertLast('t');
    catText.findNext('i'); catText.findNext('i'); catText.findNext('i'); catText.removeLast();
    catText.findNext('s'); catText.findNext('s'); catText.findNext('s'); catText.removeLast();
    catText.findNext(' '); catText.findNext(' '); catText.findNext(' ');catText.findNext(' ');catText.findNext(' ');catText.findNext(' ');catText.insertLast('a'); catText.insertLast('t');

    //I will explain why this looks bad in the TextClass::displayList() definition, line 97 of TextClass.cpp
    //Also this probably could have looked better, but it works, and I have been working on this for 6 hours, I'm tired, and mad, so pls forgive me
    std::cout << "catText: " << catText.displayList() << std::endl;

    std::cout << "catLetters's letters: ";
    for(int i=0; i < CAT; i++){
        std::cout << catLetters[i];
    } std::cout << std::endl;

    std::cout << "dogLetters's letters: ";
    for(int i=0; i < DOG; i++){
        std::cout << dogLetters[i];
    } std::cout << std::endl;


    std::cout << "Thinking Test Ended";
#endif // THINK_TEST
