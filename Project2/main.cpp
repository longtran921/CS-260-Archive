...
int main()
{
    ...
    std::cout << std::endl;
        ...
        while (start < end) {
            ...
            int temp = thinkValues[start];
            thinkValues[start] = thinkValues[end];
            thinkValues[end] = temp;

            start++;
            end--;
        }

        std::cout << "Printing out values in reverse order:" << std::endl;

        for (int i = 0; i < NUM_THINK; i++) {
            std::cout << thinkValues[i];
            if (i != NUM_THINK - 1) {
                std::cout << ", ";
            }
        }
